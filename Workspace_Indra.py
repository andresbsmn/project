...
def save(option):
    ...
    global money1_collected, money2_collected ,money3_collected,money4_collected , money1_rendered,money2_rendered, money3_rendered,money4_rendered
    if option == "save":
...
def check_if_level_completed():
    ...
    if pizza_collected is True and broccoli_collected is True and apple_collected is True and egg_collected is True and total_money_present == 4 and total_hearts_present !=0 and d_kassa <= 1.5:
        ...
        message = f'Druk op oranje knop (vooruit) om het level te voltooien.'
...
#dingen simultaan laten lopen zodat gelijktijdig lopen en sneller
        #ser = serial.Serial('COM11', 9600)
        #incoming_byte = ser.read()
        # if the incoming byte is 'H', simulate the 'a' key press
        #if incoming_byte == b'H':

         #   keyboard.press('d')

        # if the incoming byte is 'L', simulate the 'a' key release
        #elif incoming_byte == b'L':
         #   keyboard.release('d')
