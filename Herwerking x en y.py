...
#p_speler = np.array([4,3])    #herwerken naar x en y (Feniks)

p_speler_x=4
p_speler_y=3

# richting waarin de speler kijkt
#r_speler = np.array([1,0])  #herwerken naar x en y (Feniks)
...
#middelpuntcameravlak = p_speler + d_camera * r_speler  #herwerken
middelpuntcameravlak= [p_speler_x+d_camera*r_speler_x, p_speler_y+d_camera*r_speler_y]
...
def raycast(p_speler, r_straal):
    ...
    while True:
        if d_horizontaal + x * delta_h <= d_verticaal + y * delta_v:
            ...
            x += 1
            if r_straal[1] >= 0:
                if world_map[i_horizontaal_x_rounded[0], (i_horizontaal_x_rounded[1] + 1)] :
                    d_muur  = math.sqrt((i_horizontaal_x[0]-p_speler[0])**2 + (i_horizontaal_x[1]-p_speler[1])**2)
                    k_muur = kleuren[world_map[i_horizontaal_x_rounded[0], (i_horizontaal_x_rounded[1] + 1)]]
                    break
            ...
            y += 1
            if r_straal[0] >= 0:
                if world_map[(i_verticaal_y_rounded[0] + 1), i_verticaal_y_rounded[1]] :
                    d_muur  = math.sqrt((i_verticaal_y[0]-p_speler[0])**2 + (i_verticaal_y[1]-p_speler[1])**2)
                    k_muur = kleuren[world_map[(i_verticaal_y_rounded[0] + 1), i_verticaal_y_rounded[1]]]
                    break
                    #return (d_muur, k_muur)
