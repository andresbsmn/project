...
def nearest_octant(ang):
    ...
    #angle_adjusted = angDif + math.pi / 8
    #if angle_adjusted < 0:
    #    angle_adjusted += 2 * math.pi
    #return int(angDif / (math.pi/4)) % 8
    #return  round(ang / angle_range+0.5)%8
    #while (ang <-math.pi): ang += 2 * math.pi
    #while (ang > math.pi): ang -= 2 * math.pi
    #return  round((ang / math.pi + 1) / 2 * 7)
...
def volgorde_sprite_renderer():
    global  z_buffer,laser_shot  #z_buffer,
...
def sprite_renderer(sprite_x, sprite_y, sprite, nummber_sprite, z_buffer, d_object_kolom_speler,scale,sprite_bew):
    ...
    u_cameracoordinaten = (r_speler[1] * p_sprite_x_nieuw + (-r_speler[0] * p_sprite_y_nieuw))/determinant_m
    #((r_speler[1] / determinant_m) * p_sprite_x_nieuw) + ((-r_speler[0] / determinant_m) * p_sprite_y_nieuw)
    v_cameracoordinaten = ((-r_cameravlak[1] * p_sprite_x_nieuw) +(r_cameravlak[0] * p_sprite_y_nieuw))/determinant_m
    #((-r_cameravlak[1] / determinant_m) * p_sprite_x_nieuw) + ((r_cameravlak[0] / determinant_m) * p_sprite_y_nieuw)
    if v_cameracoordinaten==0:
        v_cameracoordinaten+=0.00001    #00001
    a = u_cameracoordinaten / v_cameracoordinaten  # positie op x as scherm, dus kolom
    if (sprite_bew==True):

        #print("(",sprite_x,",",sprite_y,")")
        #NTPx=p_speler[0]-sprite_x
        #NTPy=p_speler[1]-sprite_y

        #angle=math.atan2(p_speler[1] - sprite_y, p_speler[0] - sprite_x)

        angle = math.atan2(p_sprite_y_nieuw, p_sprite_x_nieuw)
        #angleNPC=math.atan2(r_clerk[1],r_clerk[0])
        #angleP=math.atan2(sprite_y,sprite_x)
        #angleNPC=math.atan2(r_clerk[1],r_clerk[0])
        #angle=angleP-angleNPC
        #print("a: ",a,"v_cameracoordinaten: ",v_cameracoordinaten)

    if (-1<=a <= 1)and (v_cameracoordinaten >=0):   #(-1<=a <= 1)and (v_cameracoordinaten >=0)
        ...
        for kolom_texture_scherm in range(0, int(breedte_sprite_scherm+0.5)):
            ...
            if (kolom < BREEDTE)  and (kolom >=0): # if(kolom < BREEDTE) and   and (kolom >= 0)

                if (z_buffer[kolom] >= d_object_kolom_speler):  #z_buffer[kolom] >= d_object_kolom_speler
                    renderer.copy(sprite, srcrect=(kolom_texture, 0, 1, hoogte_sprite_wereld), dstrect=(
                    kolom, (HOOGTE / 2) - (hoogte_sprite_scherm / 2), 1, hoogte_sprite_scherm))
                    sprite_rendered = True
                #if(sprite_bew):
                    #print(sprite_rendered)
            #else:
                #if (sprite_bew):
                    #print("breedte: ", BREEDTE, " kolom: ", kolom)
    else:
